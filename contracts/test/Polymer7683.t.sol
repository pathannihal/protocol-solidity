...
contract MockCrossL2Prover is ICrossL2ProverV2 {
    ...
    function setExpectedEvent(uint32 chainId, address emitter, bytes memory topics, bytes memory data) external {
...
contract Polymer7683Test is Test {
    ...
    function setUp() public {
        ...
        vm.prank(owner);
        polymer7683 = new Polymer7683(ICrossL2ProverV2(address(prover)), permit2, localChainId);
    }
        ...
        returns (bytes memory)
    {
        ...
        bytes memory topics = abi.encodePacked(keccak256("Filled(bytes32,bytes,bytes)"), bytes32(chainId));

        bytes memory data = abi.encode(orderId, originData, fillerData);

        prover.setExpectedEvent(uint32(chainId), emitter, topics, data);
    ...
    function _createRefundProof(
        ...
        returns (bytes memory)
    {
        bytes memory topics = abi.encodePacked(keccak256("Refund(bytes32[])"), bytes32(chainId));
    ...
    function test_handleRefundWithProof_preventReplay() public {
        ...
        assertEq(
            polymer7683.orderStatus(orderId), polymer7683.REFUNDED(), "Order should be REFUNDED after first attempt"
        );
        ...
        polymer7683.handleRefundWithProof(orderId, proof);
        assertEq(
            polymer7683.orderStatus(orderId),
            polymer7683.REFUNDED(),
            "Order should still be REFUNDED after second attempt"
        );
    }
